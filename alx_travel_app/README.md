...
- Verify payment: `GET /api/payments/verify/?tx_ref={tx_ref}`
- Booking creation automatically attaches `payment_checkout_url` and `payment_tx_ref` when available.

### Environment (.env)
```
CHAPA_SECRET_KEY=your-chapa-secret-key
CHAPA_BASE_URL=https://api.chapa.co
PAYMENT_RETURN_URL=http://localhost:8000/api/payments/verify/
...
### Celery
- Worker start: `celery -A alx_travel_app worker -l info`
- Confirmation task: `listings.tasks.send_payment_confirmation(payment_id)`

### Testing (Sandbox)
1. Create booking: `POST /api/bookings/`
2. Initiate payment: `POST /api/payments/initiate/` (or use `payment_checkout_url` from booking response)
3. Complete checkout in Chapa page
4. Verify: `GET /api/payments/verify/?tx_ref=...`
5. Confirm `Booking.status=CONFIRMED`, `Payment.status=COMPLETED`, and email log

## API Endpoints

- Listings: `GET/POST` `http://localhost:8000/api/listings/`
- Listing detail: `GET/PUT/PATCH/DELETE` `http://localhost:8000/api/listings/{id}/`
- Bookings: `GET/POST` `http://localhost:8000/api/bookings/`
- Booking detail: `GET/PUT/PATCH/DELETE` `http://localhost:8000/api/bookings/{id}/`

Request bodies follow the serializers in `listings/serializers.py`.
...
alx_travel_app/ ├── alx_travel_app/ # Main project directory │ ├── __init__.py │ ├── settings.py │ ├── urls.py │ ├── wsgi.py │ ├── asgi.py │ └── celery.py ├── listings/ │ ├── __init__.py │ ├── apps.py │ ├── models.py │ ├── migrations/ │ │ └── 0001_initial.py │ ├── views.py │ ├── urls.py │ └── tasks.py ├── requirement.txt ├── manage.py ├── .env.example ├── .gitignore └── README.md
